/*
 *  added max veclocity proberties
 *  (view.velocity_max of type math.geom.Point)
 *
 */
...
var Physics = exports = Class(function () {
	...
	this.setVelocityMax = function() {
		setterHelper(this.velocity_max, arguments, 0, 0);
		return this;
	}
	...
	this.getVelocityMax = function() { return this.velocity_max; }
	...
	this._updatePhysicsOpts = function (opts) {
		...
		if (opts.velocity_max) {
			this.velocity_max.x = opts.velocity_max.x;
			this.velocity_max.y = opts.velocity_max.y;
		}
...
Physics.tick = function (dt) {
	...
	for (var k in allPhysicsViews) {
		...
		if (view._physicsEnabled) {
			...
			if (view.velocity_max.x < Math.abs(view.velocity.x))
			  view.velocity.x = util.sign(view.velocity.x)*view.velocity_max.x;

			if (view.velocity_max.y < Math.abs(view.velocity.y))
				view.velocity.y = util.sign(view.velocity.y)*view.velocity_max.y;
